...
class PersonalityCalculationMixin:
    ...
    async def _calculate_active_profile(self, user_id: str) -> Dict[str, float]:
        ...
        Использует мультипликативную модель: База × Стиль × Эмоция × Время
        """
        ...
        # 4. Отслеживаем начало сессии если это новый пользователь или после восстановления
        self._track_session_start(user_id, profile)
        
        # 5. Обновляем время активности ПОСЛЕ всех защитных механизмов
